...
- LOCATION：**只在玩家移動到不同的「地圖地點」時輸出**，粒度與 LOCATION_DISCOVER 完全一致
  ...
  某地的門口／外圍／院子／走廊，**一律不要輸出**——那些直接寫進敘事即可，玩家仍然在同一個地點。
  移動到尚未登錄的新地點時，LOCATION 與 LOCATION_DISCOVER 必須**同時輸出**。
  ⚠️ 這個欄位決定了系統認為「哪些角色可能在場」：寫成房間名的話，整個聚落的角色都會
  比不中而無法登場（設定集裡沒有「廚房」的居民）。營地裡的角色要能出現，所在地就得是營地。
