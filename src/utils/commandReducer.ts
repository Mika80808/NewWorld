 ...
 *
 * 10 → 5：`pre_merge` 記憶是把 10 則想法原文用「；」串起來的一大塊，
 * 而 `[記憶庫]` 一次注入最近 5 條非摘要記憶——門檻 10 的時候，模型同時讀到的
 * 就是 5 大塊措辭高度雷同的想法流水帳（玩家回報「大量類似的詞彙，讓 NPC
 * 讀取記憶時過於著重在該詞彙上」）。降到 5 讓這些原文更早被濃縮封存，
 * 同時在場的原文塊數大約減半。
 *
 * ⚠️ 不要再把這個值寫死進測試——測試一律引用這個常數（先前寫死 10，
 * 一改門檻就整批紅在「未滿 10 條不觸發」這種與行為無關的地方）。
 */
export const MEMORY_MERGE_LIMIT = 5;
