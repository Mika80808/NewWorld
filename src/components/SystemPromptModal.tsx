...
export const SystemPromptModal: React.FC<SystemPromptModalProps> = ({
  ...
  return (
    <div className="responsive-modal-overlay fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4" onClick={onClose}>
      <div className="responsive-modal-panel backdrop-blur-xl w-full max-w-2xl rounded-[8px] shadow-[var(--shadow-modal)] flex flex-col overflow-hidden border border-[color:var(--tint-line)] relative z-[61] h-[80vh]" style={{ background: 'color-mix(in srgb, var(--bg-elevated) 70%, transparent)', color: 'var(--text-title)' }} onClick={e => e.stopPropagation()}>
