...
export const LorebookModal: React.FC<LorebookModalProps> = ({
  ...
  return (
    <div
      className="responsive-modal-overlay fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4"
      // 設定條目的編輯是即時寫回的，但勢力表單只存在 factionForm，按儲存前關掉會丟失，
      // 因此表單開啟時忽略遮罩點擊。
      onClick={() => { if (factionAction === null) onClose(); }}
    >
      <div
        className="responsive-modal-panel backdrop-blur-xl w-full max-w-3xl rounded-[8px] shadow-[var(--shadow-modal)] flex flex-col overflow-hidden border border-[color:var(--tint-line)] relative z-[61] h-[85vh]"
