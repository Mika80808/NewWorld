...
export function SaveSlotsModal({
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
          className="responsive-modal-overlay fixed inset-0 z-[60] flex items-center justify-center p-4"
          style={{ background: 'var(--bg-overlay)' }}
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          onClick={onClose}
        >
          <motion.div
            className="responsive-modal-panel w-full max-w-sm rounded-[12px] p-5 flex flex-col gap-4 relative z-[61]"
