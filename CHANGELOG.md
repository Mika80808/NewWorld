...
### 重構｜候選名單改打分排序＋「隨行同伴」降級為「可出場」 2026-09-06 [Claude Code]

玩家回報：「怎麼設定越來越繁瑣？」

**問題不在數量，在於旁路疊旁路。** NPC 能不能進 prompt，核心比對從頭到尾只有一句
「NPC 的主場地點字串 === 玩家當前地點」。每次出問題就在旁邊加一條 or：比不中就加
足跡、店裡的人出不來就加同城、四處跑的角色綁不住就加不限地點、常駐角色永遠不在場
就加隨行同伴。四條旁路疊在同一個沒改的地基上，語意開始互相重疊——玩家得自己分辨
釘選、隨行、不限地點、主場地點差在哪，而其中三個其實都在講同一件事。

#### 一、候選名單改打分（`utils/npcCandidates.ts`）

布林過濾換成權重。同一件事（這個人有多可能在這裡）只問一次，排序天然就是本地優先、
四處跑的墊後，上限直接砍尾巴；要加新來源只是加一個數字，不必再往那串 or 裡插分支。

| 來源 | 分數 |
|---|---|
| 可出場（`Npc.canAppear`） | 1000 |
| 主場就在這裡 | 100 |
| 遊蕩地點含此地 | 80 |
| 足跡在這裡 | 70 |
| 同城 | 40 |
...
- **地點層取最大值不加總**。主場在這裡的人同時也滿足「同城」，加總會讓他多拿 40 分，
  層與層的距離被稀釋，排序意圖跟著模糊
- **好感度只夠打破同層平手**（上限 10，層距 20~30）。好感 100 的同城店主仍排在
  好感 0 的本地居民之後——否則就倒退回「玩家在鎮上任何地方都會遇到她」
- **沒有地點理由時好感度不加分**。否則住在三座城外的摯友會出現在每一個地方的名單上

#### 二、「隨行同伴」→「可出場」（schema v10 → v11）

`Npc.isCompanion` 改名為 `canAppear`，語意同時降級：

| | 舊：隨行同伴 | 新：可出場 |
|---|---|---|
| 在場判定 | 無條件在場，`resolveOnStageNames` 併進場上名單，空 `[出場:]` 也清不掉 | 與所有人相同，只認 `[出場:]` |
| 候選名單 | **排除**（因為他是既成事實） | 永遠列入，標【常駐】排第一 |
| 完整注入 | 無條件，連關鍵字門檻也繞過 | AI 讓他登場才注入 |
| 足跡 | 跟著玩家走，AI 沒寫標記也更新 | 只有出現在 `[出場:]` 名單才更新 |
| prompt | 另開 `[隨行同伴]` 區塊宣告在場 | 候選名單裡標【常駐】＋一句指示 |

舊的旁路確實治好了「引路者被寫成神諭」，代價是它繞過整條兩階段注入鏈，讓
「AI 說誰在場」與「誰跟著玩家」永久混在一起——取消隨行時人也清不掉，因為那個
狀態已經寫進 `appearingNpcs` 存檔了。降級之後在場判定只剩一個來源。

⚠️ **代價**：常駐角色現在有可能被 AI 判斷成「這一幕他不在」。護欄是候選名單底下
那句指示（「除非劇情上有明確理由，否則本回合應該讓他在場並寫進 `[出場:]`；
他有實體，不要寫成憑空傳來的聲音」）。真的又出現角色消失，第一個要檢查的是那句話
還在不在，而不是再加一條旁路。

遷移直接沿用舊值，勾過隨行的角色仍然每回合排在名單最前面。匯入端同時收舊的
`isCompanion` 鍵——舊的匯出檔還帶著它，認不得就等於常駐角色跨檔之後全部掉光。

#### 三、順手修的兩件事

- `[Pinned NPCs]` 改名 `[角色補充資料]`，收的人從「釘選／隨行」改成
  「釘選**或本回合真的在場**」。這一段的用途是「設定集裡沒有條目的角色的兜底資料」，
  而任何角色（`handleAddNpc` 建的、舊存檔帶進來的）只要沒有條目、又被 AI 請上台，
  模型就只拿得到一個名字——在場才是該給資料的理由，隨不隨行不是
- `resolveOnStageNames` 整支移除。在場名單再也沒有例外，`SceneNpcsWidget` 與
  `promptBuilder` 兩邊都直接讀 `appearingNpcs`

測試 +25 條（`npcCandidates.test.ts` 19 條、migration 6 條），既有測試同步改寫；
`npm run lint` 綠、`npm test` 827 passed。
