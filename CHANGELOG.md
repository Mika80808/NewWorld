...
`currentGoals`（便條紙上的「當前目標」）的完整生命週期是——
助理 GM 的 `updateAdventureState` 每 3 回合整理一次寫進去、GoalsPanel 顯示、
玩家可以就地手動改寫（`onEditGoals`）、`buildSaveSnapshot` 存進雲端。

唯獨主 GM 從來沒拿到過：`BuildPromptDeps` 裡根本沒有這個欄位。

後果是玩家手改目標等於對著空氣講話。把目標改成「先不要接委託，我要去找伊凡」，
GM 完全不知情，下一回合照樣推委託。

**修法**：`[當前目標（玩家正在追求的方向，可能由玩家手動改寫）]` 區塊，逐條列出。

- 標題寫明「可能由玩家手動改寫」——不加這句，模型會以為那是自己上一輪寫的東西，
  玩家改過的部分就沒有特別的份量
- 排在 `[進行中任務]` **之前**：任務是接下的委託，目標是玩家自己在追的方向，
  後者可能與任何委託無關（「先去找伊凡」），先讀到它才不會被委託清單帶走
- 位置在動態區不在靜態前綴——目標逐回合會變，放進前綴會讓 context caching 失效
- 沒有目標時整段省略，不補「（無）」佔位（注意事項 19）

新增 5 個測試，其中 3 個確認過「拿掉注入就會紅」。

本次盤點另外找到四處也是「該讀沒讀到」，這次刻意不動（任務的
`description` / `reward`、`cartFare`、`mapStatus`、`Npc.lastSeenDate`、
已完成任務不注入），留待各自評估。

---

### Bug 修正｜AI 讀不到世界節日 2026-09-19 [Claude Code]

玩家回報：AI 讀不到世界節日。

**原因**：`constants.MONTHS_DATA` 的月份雅稱與節慶描述（十二個月各一組
`elegant` / `desc`）**只流向 UI**——右欄的 World Memory Widget 與狀態列的
tooltip。`promptBuilder` 的 `[Current State]` 從頭到尾只給：

```
Time: 1024年4月15日 12:00 | Weather: 晴朗
```

玩家在畫面上看得到「四月・雙月之月：月光特別明亮⋯霧光許願夜與星織之夜」，
AI 拿到的只有一個數字 `4`。節慶對它等於不存在，於是它筆下的四月與十月
毫無差別——而這份資料明明一直都在存檔外的常數裡躺著。

**修法**：`[Current State]` 多一行

```
Month: 四月・雙月之月｜本月節慶與時節：月光特別明亮，彷彿放大一般。霧光許願夜與星織之夜。
```

- 查詢收在 `utils/worldCalendar.ts` 的 `monthInfo()`，App.tsx 原本自己寫的
  `find(...) || MONTHS_DATA[0]` 一併改走這裡——fallback 只留一份
- 月份不合法（舊存檔、指令寫壞）時退回一月而不拋錯：時間顯示與 prompt
  都不該因為一個怪數字就整段消失
- 「本月節慶與時節：」這幾個字是刻意的。只丟一句描述的話，模型不見得知道
  那是這個月會發生的事，可能當成背景設定讀過去
- 位置在 `[Current State]`（動態區）而不是靜態前綴——節慶逐月變動，
  放進前綴會讓 context caching 每個月失效一次

新增 5 個 `worldCalendar` 測試與 3 個 `promptBuilder` 注入測試。
注入測試確認過「拿掉那一行就會紅」，不是裝飾性斷言。

---

### Bug 修正｜手機看不見冒險摘要 2026-09-18 [Claude Code]

玩家回報：手機打開左側選單，便條紙上的「冒險摘要」是空的。

**原因是 flex 壓扁，不是資料沒生出來。** 左右兩欄（桌機側欄與手機抽屜）都是
「高度受限的 `flex flex-col` + `overflow-y-auto`」，這種容器的子項預設
`flex-shrink: 1`——內容超出時**子項先被壓扁，容器不會變成可捲動**。

⚠️ 關鍵在子項能被壓到多小。flex 子項的 `min-height: auto` 通常由內容的最小
尺寸兜底（一顆單行按鈕壓不到比一行字還矮），所以多數東西只是「擠一點」。
但**子項本身是捲動容器時（`overflow` 不是 visible），自動最小尺寸是 0**——
可以被壓到任意矮，而且把超出的部分自己裁掉，不留任何痕跡。

`GoalsPanel` 的根節點正是 `overflow-hidden`，而冒險摘要排在那張便條紙的最下面。
手機抽屜比桌機欄矮，於是只有手機會裁到它。

**實測**（Playwright + 390×664 直式手機）：卡片被壓到 172px 而內容需要 265px，
摘要整段（253–321px）落在卡片可見底部（240px）之外；抽屜還因為子項讓位而
`scrollHeight === clientHeight`，**連往下滑都救不回來**。加上 `flex-shrink: 0`
之後卡片回到 265px、摘要可見、抽屜恢復可捲。

**修法**：`.scroll-col > * { flex-shrink: 0 }`，掛在桌機左右欄、手機左右抽屜與
快速選項清單五處。快速選項是預防性的——量測顯示它的按鈕受 min-content 保護、
本來就沒被壓扁，但容器形狀相同，之後放進一個帶 `overflow-hidden` 的子項就會中。

`scrollColShrink.test.ts` 掃 App.tsx，擋「新增這類容器卻忘了掛 class」——
那種漏掉不會報錯，只會讓某個 widget 的下半截在某些螢幕尺寸悄悄消失。

---

### 新功能｜「回到最新訊息」置底按鈕 2026-09-09 [Claude Code]

對話區往上捲看歷史之後，要回到最新訊息只能一路往下滑。新增一顆浮動按鈕
（`components/ScrollToBottomButton.tsx`），一按直接回到底部。

**只在捲離底部時出現**。一直掛著會擋住故事，而且人在底部時按了也沒有作用。

**位置貼在輸入框的右上角外側**，不是用固定的 bottom 位移：對話區在桌機是被
輸入框蓋住底部的（composer 是 `absolute bottom-0`），手機則是排在流程裡——
掛在輸入框上，兩種版面才不必各算一次高度，快速選項展開時也會自己跟著上移。

**門檻不是 0**：捲到最底時各家瀏覽器的 `scrollHeight` / `clientHeight` 會有
次像素誤差，用 `=== 0` 判定會讓按鈕在底部閃爍。判定收在
`utils/visibleMessages.isScrolledToBottom()`（門檻 120px，約一則短訊息的高度），
邊界與負數誤差都有測試釘住。

順帶把對話區 `onScroll` 的 DEV／正式版兩個分支收成同一支 `onScrollTick`——
load-more 先前在兩個分支各寫一次，再加一件事（這次的底部判定）就會變成
兩邊各漏改一半。

新增 5 個組件測試與 7 個 `isScrolledToBottom` 測試。
...
完全相同的區塊，扣掉 state 接線那種本來就會長一樣的，剩下的都是真的重複。

共同形狀是**同一份內容存在兩個地方**：桌機側欄與手機抽屜、摺疊與展開、
兩個分頁的同一種卡片。這種重複不會報錯，代價是改一個 prop 或一個顏色
得記得改兩個地方，而漏掉的那一邊沒有任何東西會提醒你——LorebookModal
的關鍵字膠囊就已經漂開了（見下）。

**1. App.tsx（3204 → 3139 行）**

- `uploadSnapshot()`：手動儲存與自動存檔各寫一份相同的 20 行上傳流程，
  只差成功時要不要提示與失敗訊息措辭
- `rightPanelWidgets`：右欄三個 Widget 在桌機側欄與手機右抽屜是逐字相同的 JSX
- `SidebarWidgetButton`：裝備／消耗品的開關按鈕出現四次（桌機兩顆、手機兩顆），
  每份都完整抄了底色／邊框／hover／徽章。兩種版面只差 ref 與展開箭頭
- `equipmentListEl` / `consumableListEl`：桌機浮動面板與手機就地展開的外框不同，
  裡面的 props 完全一樣

**2. LorebookModal（1268 → 1142 行）**

「通用 Grid」與「地點 Grid」各寫一份幾乎逐字相同的 75 行唯讀卡片，
抽成 `LorebookEntryCard`。

⚠️ 兩份**已經漂開了**：通用版把 `selective` 的次要關鍵字加上 `+` 前綴以示區別，
地點版把主要與次要混在一起。合併時採用通用版——次要關鍵字是「必須同時命中」
的條件，玩家分不出來就設不對。地點分頁的次要關鍵字現在也有 `+` 前綴。

同時修掉半吊子的 debounce：搜尋框有 300ms 延遲，但只有通用 Grid 讀
`debouncedSearch`，地點與 NPC 兩個 Grid 讀的是輸入框即時值——那兩個分頁
每打一個字就整份重篩重繪，而設定集動輒上百條。三個 Grid 統一改讀 debounced。

**3. ItemCard**

`EquipmentList` 與 `ConsumableList` 各寫一份相同的卡片殼（邊框、點一下展開、
名稱列、說明、展開動畫與分隔線）。差別只有數量徽章與展開後那排按鈕。
順帶把五顆操作按鈕的 hover 配對收成 `ItemActionButton`——成對的
onMouseEnter/onMouseLeave 最容易只改一邊，留下 hover 卡住的顏色。

**4. DiaryKeywordChips**

摺疊與展開兩種版面各貼一份相同的 14 行關鍵字膠囊，連 `scanKeywords` 的
三元運算式都一字不差。

---

行數上是 +495 / −463（淨 +32）：四個新組件本身帶了說明註解，
但五處重複的來源收斂成了一份。除了上面兩處明講的行為變更
（地點分頁的 `+` 前綴、地點與 NPC 分頁改吃 debounce），畫面與行為不變。

lint、tsc、893 個測試與 `vite build` 全部通過。
...
```css
@media (max-width: 640px) { :root { font-size: 14px; } }   /* 瀏覽器預設是 16px */
```

手機版**主動把根字級縮到比預設還小**，所有 `rem` 尺寸（含 Tailwind 的
`text-sm` / `text-xs`）跟著縮 12.5%。對一個以讀故事為主的遊戲這是反的——
螢幕小、閱讀距離近，需要的是更大的字。已改為 16px。

而且這條規則在 `index.css` 裡**重複了兩次**（註解還是壞的），一併清除。
下方觸控點擊區那段引用「手機根字級是 14px」的註解同步更新。

**② 羊皮紙擬物主題（切片，尚未成為預設）**

全部收在 `[data-theme="parchment"]` 之下，**預設深色主題完全不受影響**，
方向不對可整段刪除。做法是覆寫既有的 80 幾個顏色變數——全站 994 處
`var(--*)` 引用會自動跟著變，不需要改組件的顏色。

新增擬物 token（對應參考圖的 Tokens 欄）：`--tac-radius-*`、`--tac-border-*`、
四階 `--tac-shadow-*`（none / inner / lift / float）、`--tac-highlight`。
以及三個材質類別 `.tactile-raised` / `.tactile-float` / `.tactile-sunken`
與紙張顆粒 `.tactile-paper`（SVG feTurbulence，不需要圖片檔）。

⚠️ **擬物與毛玻璃是相衝的視覺語言**，這個主題底下一律關掉 `backdrop-filter`，
否則會變成半透明塑膠感，兩邊的缺點都拿到。對話泡泡的掃光動畫在紙面上像油漬，
也一併關閉。閱讀行距提到 1.95。

已套用切片：對話泡泡、當前場景人物 Widget。

**③ 切片暴露出硬編碼色碼的實際危害**

第一次實測截圖時，「當前場景人物」整塊**維持深灰、文字變成深底深字完全看不見**
——因為它的底色寫死成 `rgba(10,15,10,0.55)` 而非 CSS 變數。這正是顏色規則
存在的理由。該組件的三處硬編碼（容器底、內卡邊框、好感度膠囊）已改為變數。

> 全面換皮的主要工作不是寫新樣式，而是把散落的**64 處硬編碼色碼**挖出來——
> 不處理的話換主題後會一塊一塊地壞掉。
